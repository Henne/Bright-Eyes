...
for i in soa_*.cpp; do
	#remove soa_ prefix from filenames
	cp ${i} ${DRIVE_C}/src/${i##soa_}
	#remove soa_ prefix in cpp-files
	sed -i 's/soa_//g' ${DRIVE_C}/src/${i##soa_}
done
for i in soa_*.h; do
	#remove soa_ prefix from filenames
	cp ${i} ${DRIVE_C}/src/${i##soa_}
	#remove soa_ prefix in h -files
	sed -i 's/soa_//g' ${DRIVE_C}/src/${i##soa_}
done

cp common.h ${DRIVE_C}/src
cp vgame_en.h ${DRIVE_C}/src

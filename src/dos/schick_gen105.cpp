...
int schick_farcall_gen105(unsigned segm, unsigned offs, unsigned ss)
{
	...
	if (segm == 0xb6b)
	{
		...
		if (offs == 0x184) {
			unsigned short offset=real_readw(ss, reg_sp);
			unsigned short n=real_readw(ss, reg_sp+2);
			unsigned short color=real_readw(ss, reg_sp+4);
			unsigned short space=real_readw(ss, reg_sp+6);
		...
		if (offs == 0x1f3)
		{
			D1_GFX("DrawPic(Dest=0x%x:0x%x, X=%d, Y=%d, ..., Breite=%d, Höhe=%d, Src=0x%x:0x%x, Mode=%d);\n",
				real_readw(ss, reg_sp+2), real_readw(ss, reg_sp),
				real_readw(ss, reg_sp+4), real_readw(ss, reg_sp+6),
				real_readw(ss, reg_sp+20), real_readw(ss, reg_sp+22),
				real_readw(ss, reg_sp+26), real_readw(ss, reg_sp+24),
				real_readw(ss, reg_sp+28));
			return 0;
		}
		if (offs == 0x2e3) {
			...
			D1_GFX("FillRect(segment=0x%x, offset=0x%x, color=0x%x, width=%d, height=%d);",
				seg, off, real_readw(ss, reg_sp+4),
				real_readw(ss, reg_sp+6), real_readw(ss, reg_sp+8));
		...
		if (offs == 0x39f) {
			D1_GFX("_39F(src=0x%x:0x%x, dst=0x%x:0x%x, a=%d, b=%d, c=%d);\n",
				real_readw(ss, reg_sp+2), real_readw(ss, reg_sp),
				real_readw(ss, reg_sp+6), real_readw(ss, reg_sp+4),
				real_readw(ss, reg_sp+8), real_readw(ss, reg_sp+10),
				real_readw(ss, reg_sp+12));
			return 0;
		}
